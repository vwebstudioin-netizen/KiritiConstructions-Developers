...
export default function EnquiriesPage() {
  ...
  useEffect(() => { getAllEnquiries().then((e) => setEnquiries(e.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()))) }, [])
  ...
  return (
    <div>
      ...
      <p className="font-body text-muted text-sm mb-6">{enquiries.filter((e) => e.status === 'new').length} new</p>
          ...
          <div key={enq.id} className="admin-card">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <select value={enq.status} onChange={(e) => handleStatus(enq.id, e.target.value as EnquiryStatus)} className="input-field text-xs py-2 w-32">
                ...
                <button onClick={() => handleDelete(enq.id)} className="p-2 text-red-400 bg-red-50 rounded-xl hover:bg-red-100 transition-colors"><FiTrash2 size={16} /></button>
              </div>
            </div>
          </div>
        ))}
        {filtered.length === 0 && <div className="text-center py-16 admin-card"><p className="font-body text-muted">No {filter === 'all' ? '' : filter} enquiries.</p></div>}
      </div>
    </div>
  )
}
